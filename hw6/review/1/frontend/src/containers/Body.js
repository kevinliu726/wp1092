...
const Body = () => {
  ...
  const handleQuery = async () => {
    ...
    else if(messages.length == 0) addRegularMessage(`${queryType} (${queryString}) Not Found`)
  ...
  return ( <
    ...
    >
    <
    FormControlLabel value = "subject"
    control = {
      < Radio color = "primary" / >
    }
    label = "Subject" /
    ...
    /FormControl> <
    /StyledFormControl> <
    TextField placeholder = "Query string..."
    value = {
      queryString
    }
    onChange = {
      handleChange(setQueryString)
    }
    style = {
      {
        flex: 1
